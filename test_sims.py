...
def test_saturating_laws_separate_the_four_leaf_pair_and_are_not_idempotent():
    """A saturating mean is outside the translation-equivariant pointwise family:
    it separates ((AB)(CD)) from ((AC)(BD)) at every gain, and G(u,u) != u."""
    laws = Laws(np.random.default_rng(2))
    for name in ("sat_mean", "sat_tied"):
        assert _fourleaf(laws.get(name)) > 0.1, name
        assert not np.allclose(laws.get(name)(F, F), F), name


def test_sum_only_law_is_blind_to_daughter_disagreement_and_the_gate_is_not():
    """The discriminating experiment: two daughter pairs with the same SUM but a
    different pointwise disagreement.  A sum-only law phi(u+v) gives identical
    composites; the soft-min gate does not."""
    laws = Laws(np.random.default_rng(2))
    u, v = F, G
    delta = 0.7 * H
    u2, v2 = u + delta, v - delta            # same sum, larger |u - v|
    assert np.allclose(laws.sat_mean(u, v), laws.sat_mean(u2, v2))
    assert not np.allclose(laws.gate_norm(u, v), laws.gate_norm(u2, v2), atol=1e-3)


def test_equivariant_family_is_second_order_blind_to_pairing():
    """For any pointwise commutative translation-equivariant gate with h(0)=0 the
    four-leaf separation is fourth order in signal amplitude (second- and
    third-order terms cancel).  Check the exact corrected Shannon gate."""
    g = lambda u, v: (u + v) / 2 - ((u - v) / 2) * np.tanh((u - v) / 2)
    lex = make_lexicon(4, np.random.default_rng(9)); ta, tb, n = balanced_pair()
    sep = []
    for eps in (0.125, 0.0625, 0.03125):
        a = compose_tree(ta, lex * eps, g); b = compose_tree(tb, lex * eps, g)
        sep.append(np.linalg.norm(a - b) / eps ** 4)
    assert max(sep) / min(sep) < 1.5, sep          # ratio to eps^4 is flat


def test_meld_properties():
    """Commutative, non-associative, bounded, not idempotent, disagreement-
    sensitive, and separating the four-leaf pair at both temperature limits."""
    for beta in (0.03, 1.0, 30.0):
        laws = Laws(np.random.default_rng(2), beta=beta)
        c = laws.meld
        assert np.allclose(c(F, G), c(G, F))
        assert np.linalg.norm(c(c(F, G), H) - c(F, c(G, H))) > 1e-2
        assert np.abs(c(F, G)).max() <= 1.0
        assert not np.allclose(c(F, F), F)
        assert _fourleaf(c) > 0.2, (beta, _fourleaf(c))
    laws = Laws(np.random.default_rng(2), beta=1.0)
    delta = 0.7 * H
    assert not np.allclose(laws.meld(F, G), laws.meld(F + delta, G - delta), atol=1e-3)


def test_lin_tied_properties():
    """Meld without the transfer: commutative, non-associative, not idempotent,
    disagreement-sensitive, and separating the four-leaf pair at SECOND order --
    the shared weights, not the saturating transfer, are what lift the law out
    of the pointwise translation-equivariant class (manuscript Sec 4.9.1)."""
    laws = Laws(np.random.default_rng(2), beta=1.0)
    c = laws.lin_tied
    assert np.allclose(c(F, G), c(G, F))
    assert np.linalg.norm(c(c(F, G), H) - c(F, c(G, H))) > 1e-2
    assert not np.allclose(c(F, F), F)
    delta = 0.7 * H
    assert not np.allclose(c(F, G), c(F + delta, G - delta), atol=1e-3)
    lex = make_lexicon(4, np.random.default_rng(9)); ta, tb, n = balanced_pair()
    sep = []
    for eps in (0.4, 0.2, 0.1):
        a = compose_tree(ta, lex * eps, c); b = compose_tree(tb, lex * eps, c)
        sep.append(np.linalg.norm(a - b) / eps ** 2)
    assert max(sep) / min(sep) < 1.5, sep          # ratio to eps^2 is flat
    # and the second-order term is c W { W[(A-D)o(B-C)] - (W(A-D)) o (W(B-C)) }, c = beta/8
    A, B, C, D = lex; W = laws.Wt; eps = 0.05
    pred = (1 / 8) * (W @ (W @ ((A - D) * (B - C)) - (W @ (A - D)) * (W @ (B - C))))
    diff = (compose_tree(ta, lex * eps, c) - compose_tree(tb, lex * eps, c)) / eps ** 2
    assert np.corrcoef(diff, pred)[0, 1] > 0.99


def test_meld_without_W_leaks_no_order():
    """Removing the tied weights (W = I) does NOT give an order-leaking law --
    the law tanh(kappa * mixture) is symmetric in its arguments.  Order leaks
    only when the weights are UNTIED (the `rnn` law), which is the ablation
    that the commutativity argument actually needs."""
    laws = Laws(np.random.default_rng(2), beta=1.0)
    noW = lambda u, v: np.tanh(laws.meld_gain * gate_norm(u, v, 1.0, 2.0))
    lex = make_lexicon(5, np.random.default_rng(3))
    a, b, n = order_pair(2)
    assert np.allclose(compose_tree(a, lex[:n], noW), compose_tree(b, lex[:n], noW), atol=1e-9)
...
def test_gate_is_order_blind_and_rivals_are_not():
    ...
    lex = make_lexicon(5, np.random.default_rng(3))
    a, b, n = order_pair(2)
    for name in ("gate", "gate_norm", "additive", "mean_pool", "sat_mean", "sat_tied", "meld", "lin_tied"):
        f = laws.get(name)
