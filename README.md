...
python test_sims.py      # 32 analytic self-checks, a few seconds
```
...
| File | Role |
|---|---|
| `gate_core.py` | Sim A. Gabor atoms, the Rényi gate, the offset-corrected gate, the rival laws (incl. fixed mean-pooling, Meld and Meld's linear form), tree builders, the balanced four-leaf contrast. |
...
| `ablate_lin_tied.py` | Standalone Meld ablations (no transfer; no W) with `run_sim.py`'s seeds; modifies nothing |
...
is evaluated pointwise by grid minimization over 1,201 knots. Ten laws are
compared:
...
| `sat_tied` | `tanh(1.5·W(u+v)/2)`, W orthogonal — `rnn` with W_u = W_v | yes | **no** | saturating, sum-only |
...
| `lin_tied` | `W [λ*u + (1−λ*)v]` — Meld without the transfer, κ = 1 | yes | **no** | content-dependent, linear |
...
**Meld** (the proposed neural binding operation; Merge is the formal operation, Meld a candidate realization of its binding factor). Its mixture is the
...
weights (commutativity) and a saturating transfer (boundedness at any output
gain). It reduces to `sat_tied` as β → 0 and to a
...
**`lin_tied`** is Meld with the saturating transfer removed (κ = 1). It is the
ablation that shows where Meld's algebraic content sits. The tied weights alone
already leave the pointwise translation-equivariant class: `W[λ*u + (1−λ*)v]` is
not idempotent (`G(u,u) = Wu ≠ u`) and separates the four-leaf pair at *second*
order in leaf amplitude, with second-order term
`c·W{ W[(A−D)∘(B−C)] − (W(A−D))∘(W(B−C)) }`, c = β/8, which vanishes only for a
permutation W (`test_sims.py` checks the ε² scaling and this term). It matches
Meld on every axis — 1.000/1.000/0.996/0.966/0.893 on the ladder, order at
chance, four-leaf pair 1.000 (d′ 8.5 vs Meld's 9.1). Conversely, Meld with
W = I (`ablate_lin_tied.py`) leaks no order either. So the soft minimum and the
tied weights are what the algebra forces; the saturating transfer bounds the
composite at any output gain and is what cortical neurons do, and is retained on
those grounds (manuscript §4.9.1). `lin_tied` is appended last in `LAWS`, so
every decoder seed of the earlier laws is unchanged and their numbers reproduce
exactly.
...
BRACKETING accuracy @ 0 dB (rows=laws, cols=depth 1..5)
  ...
  lin_tied   1.000 1.000 0.996 0.966 0.893
ORDER accuracy @ 0 dB
  ...
  lin_tied   0.496 0.497 0.497 0.506 0.499
d' @ 0 dB, bracketing (||m1-m2||/sigma)
  ...
  lin_tied    13.57   8.77   6.03   4.02   2.91
