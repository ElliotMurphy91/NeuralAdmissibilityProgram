...
LAWS = ["gate", "gate_norm", "additive", "tensor", "rnn", "mean_pool"]
N_ITEM = 60
...
for bi, b in enumerate(BETAS_SWEEP):
    for name, fn in (("gate_norm", gate_norm), ("gate", gate)):
        Gf = lambda u, v, b=b, fn=fn: fn(u, v, b, ALPHA0)
...
for bi, b in enumerate(BETAS_SWEEP):
    print(f"  beta={b:5}: {ACCb['gate_norm'][bi,s0].mean():.3f} | {ACCb['gate'][bi,s0].mean():.3f}")
