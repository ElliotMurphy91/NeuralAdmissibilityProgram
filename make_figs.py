...
C = {"gate": "#12807F", "gate_norm": "#7FC6BE", "additive": "#9AA5B1",
     "tensor": "#E27D29", "rnn": "#3C53A4", "mean_pool": "#B5893F"}
...
ORDER = ["tensor", "rnn", "mean_pool", "gate_norm", "gate", "additive"]
...
for nm in ORDER:
    ...
    axA.plot(D, m, color=C[nm], lw=1.5, marker="o", ms=3.2, mec="white", mew=0.6,
             label=PRETTY[nm], zorder=3, ls=(0, (3, 1.5)) if nm == "mean_pool" else "-")
...
axA.set_xlim(0.7, 7.4)
axA.legend(loc="center right", bbox_to_anchor=(1.0, 0.55))
tag(axA, "a", dx=-0.115)

# ---- b: gate heatmap depth x SNR
axB = fig.add_subplot(gs[0, 2])
gi = LAWS.index("gate")
H = A[gi, :, :].mean(2).T                                  # (SNR, depth)
im = axB.imshow(H, origin="lower", aspect="auto", cmap=TEAL_CMAP, vmin=0.5, vmax=1.0,
                extent=[0.5, 5.5, -0.5, len(S) - 0.5])
cs = axB.contour(np.linspace(1, 5, 5), np.arange(len(S)), H, levels=[THR],
                 colors="#E27D29", linewidths=1.6, linestyles="--")
axB.set_yticks(range(len(S))); axB.set_yticklabels([f"{int(s):+d}" for s in S])
axB.set_xticks(D); axB.set_xlabel("depth  $d$"); axB.set_ylabel("SNR (dB)")
axB.set_title("Gate: powered region", loc="left")
cb = fig.colorbar(im, ax=axB, pad=0.03, fraction=0.06)
...
for si, sh in zip([list(S).index(v) for v in [10., 5., 0.]], ["#0a4d4c", "#2f9c98", "#9ad6d1"]):
    g = (A[gi, :, si] - A[LAWS.index("additive"), :, si])
    m, e = ci(g.T, 0)
    axC.fill_between(D, m - e, m + e, color=sh, alpha=0.18, lw=0)
    axC.plot(D, m, color=sh, lw=1.5, marker="o", ms=3.0, mec="white", mew=0.6,
             label=f"{int(S[si]):+d} dB")
axC.set_xticks(D); axC.set_xlabel("depth  $d$")
axC.set_ylabel("gate $-$ additive  (acc.)")
axC.set_title("The gap closes, not opens", loc="left")
axC.legend(loc="upper right", bbox_to_anchor=(1.02, 1.06))
tag(axC, "c")
...
for nm in ORDER:
    ...
    axD.plot(D, m, color=C[nm], lw=1.5, marker="o", ms=3.2, mec="white", mew=0.6, zorder=3,
             ls=(0, (3, 1.5)) if nm == "mean_pool" else "-")
...
# ---- e: representational divergence
axE = fig.add_subplot(gs[1, 2])
DV = R["DIV"]
for j, (mk, lab) in enumerate([("o", "$((AB)C)$"), ("s", "$(A(BC))$")]):
    m, e = ci(DV[:, :, j].T, 0)
    axE.errorbar(D + (j - 0.5) * 0.08, m, yerr=e, color=C["gate"], lw=1.4,
                 marker=mk, ms=3.4, mfc="white" if j else C["gate"], mew=0.9,
                 capsize=1.6, elinewidth=0.8, label=lab)
axE.set_xticks(D); axE.set_xlabel("depth  $d$")
axE.set_ylabel("$r$ (gate, additive)")
axE.set_ylim(0.63, 0.94)
axE.set_title("Divergence is bracketing-blind", loc="left")
axE.legend(loc="lower left", ncol=2, columnspacing=0.7, handletextpad=0.4)
tag(axE, "e")
