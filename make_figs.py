...
C = {"gate": "#12807F", "gate_norm": "#7FC6BE", "additive": "#9AA5B1",
     "tensor": "#E27D29", "rnn": "#3C53A4", "mean_pool": "#B5893F", "sat_tied": "#8E3B6B", "meld": "#C0392B", "lin_tied": "#E8A39A"}
PRETTY = {"gate": "Rényi gate  $\\oplus_{S_2,\\beta}$",
          ...
          "sat_tied": "tied-weight  $\\tanh(1.5\\,W(u+v)/2)$",
          "lin_tied": "Meld, no transfer  $W[\\lambda^{\\star}u+(1-\\lambda^{\\star})v]$",
...
ORDER = ["tensor", "rnn", "meld", "lin_tied", "sat_tied", "mean_pool", "gate_norm", "gate", "additive"]
ORDER = [nm for nm in ORDER if nm in LAWS]        # tolerate a results.npz produced before lin_tied was added
...
for nm in ORDER:
    ...
    axA.plot(D, m, color=C[nm], lw=2.0 if nm == "meld" else 1.5, marker="o", ms=3.2, mec="white", mew=0.6,
             ...
             ls=(0, (3, 1.5)) if nm in ("mean_pool", "lin_tied") else (0, (1, 1.2)) if nm == "sat_tied" else "-")
