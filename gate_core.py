...
Implements, per the accompanying manuscript:
  ...
  - the rival composition laws, incl. fixed mean-pooling, the
    tied-weight saturating unit, Meld, and Meld's linear form   (Sec 4.9.1, 7.2)
...
class Laws:
    ...
    def __init__(self, rng, beta=1.0, alpha=2.0):
        ...
        # tied-weight saturating law: the commutative member of the recurrent
        # family (both daughters enter through the same synapses).  Drawn last so
        # that every earlier draw, and every published number, is unchanged.
        self.Wt = np.linalg.qr(rng.normal(size=(NPT, NPT)))[0]
        self.sat_gain = 2.0
        self.meld_gain = 2.0     # kappa: output gain before saturation
    ...
    def sat_mean(self, u, v):
        """Saturating mean: tanh(g (u+v)/2) / tanh(g).  Commutative, non-associative,
        bounded by saturation, NOT translation-equivariant, and not idempotent."""
        return np.tanh(self.sat_gain * 0.5 * (u + v)) / np.tanh(self.sat_gain)

    def sat_tied(self, u, v):
        """Tied-weight saturating recurrent law: tanh(W (u+v)/2) with W orthogonal.
        This is the `rnn` law with W_u = W_v, i.e. the recurrent family's
        commutative member."""
        return np.tanh(1.5 * (0.5 * (u + v)) @ self.Wt.T)
    ...
    def meld(self, u, v):
        """Meld(u, v) = tanh( kappa * W [ lam* u + (1 - lam*) v ] ),
        lam* the entropy-optimized mixture of the corrected gate (a pointwise
        soft minimum, penalising daughter disagreement), W orthogonal tied weights
        (commutativity), tanh a saturating transfer (boundedness at any output
        gain).  Reduces to `sat_tied` as beta -> 0 and to a saturated tied
        minimum as beta -> inf.  The algebraic content lies in the mixture and
        the tied weights: see `lin_tied`, which drops the transfer and matches
        Meld on every contrast."""
        return np.tanh(self.meld_gain * gate_norm(u, v, self.beta, self.alpha) @ self.Wt.T)

    def lin_tied(self, u, v):
        """Linear tied-weight mixture:  W [ lam* u + (1 - lam*) v ],  kappa = 1.
        Meld without the saturating transfer.  Commutative, non-associative,
        order-blind, disagreement-sensitive, bounded at kappa <= 1 and NOT
        idempotent (G(u,u) = W u != u).  Because W is not a permutation it leaves
        the pointwise translation-equivariant class, so the balanced four-leaf
        pair is separated at second order in leaf amplitude (the pointwise gates
        separate it at fourth).  Drawn from the same W as `sat_tied` and `meld`,
        so no earlier draw is disturbed."""
        return gate_norm(u, v, self.beta, self.alpha) @ self.Wt.T

    def get(self, name):
        return {"gate": self.gate, "gate_norm": self.gate_norm,
                "additive": self.additive, "tensor": self.tensor,
                "rnn": self.rnn, "mean_pool": self.mean_pool,
                "sat_mean": self.sat_mean, "sat_tied": self.sat_tied,
                "meld": self.meld, "lin_tied": self.lin_tied}[name]


# lin_tied is appended LAST so that every decoder seed of the earlier laws
# (7*di + si + 31*li, 5000 + 31*li) is unchanged and their published numbers reproduce.
NAMES = ["gate", "gate_norm", "additive", "tensor", "rnn", "mean_pool", "sat_mean", "sat_tied", "meld", "lin_tied"]
